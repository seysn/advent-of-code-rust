    ...
    for l in input.lines() {
        if re_others.is_match(l) {
            ...
            match &caps[2] {
                ...
                "LSHIFT" => wires.insert(caps[4].to_string(), Instruction::LeftShift(caps[1].to_string(), caps[3].parse().unwrap())),
                "RSHIFT" => wires.insert(caps[4].to_string(), Instruction::RightShift(caps[1].to_string(), caps[3].parse().unwrap())),
                _ => unreachable!(),
            };
